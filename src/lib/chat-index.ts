...
type EmbeddingRow = {
  source_type: "post" | "paper" | "case_study";
  source_id: string;
  title: string;
  url_path: string;
  chunk_text: string;
...
// paper, and case study. Run from /admin/chat-index whenever content
// changes — there's no automatic trigger, so a stale index just means the
// chat answers from slightly older content, not broken content.
